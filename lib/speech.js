...
class SpeechCapture {
  ...
  async start() {
    ...
    // A stop()/cancel() click can land while getUserMedia is still resolving,
    // before isRecording is set — without this it would hit the isRecording
    // guard and be dropped, leaving the UI stuck in its recording state.
    // Queue the call and replay it once start() settles.
    this._starting = true;
    this._pending = null;
    try {
      await this._doStart();
  ...
  stop() {
    if (this._starting) { this._pending = 'stop'; return; }
  ...
  // Abort the capture: tear everything down like stop(), but discard the
  // audio and never fire onEnd — nothing downstream (transcribe/submit) runs.
  cancel() {
    if (this._starting) { this._pending = 'cancel'; return; }
    if (!this.isRecording) return;
    this.isRecording = false;
    ...
    if (this.mediaRecorder && this.mediaRecorder.state !== 'inactive') {
      this.mediaRecorder.onstop = () => this._stopMediaStream();
      this.mediaRecorder.stop();
    } else {
      this._stopMediaStream();
    }
