...
//
// Returns { kind: 'not_configured' | 'offline' | 'privacy' | 'bad_key' | 'proxy' | 'internal',
...
function describeApiError(err, actionLabel) {
  ...
  if (code === 'PRIVACY') {
    return {
      kind: 'privacy',
      openSettings: true,
      text: 'Privacy mode is on — this feature sends paper content to the LLM, so it is disabled. Turn privacy mode off in Settings.',
    };
  }
