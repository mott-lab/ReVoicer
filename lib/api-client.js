...
class ApiClient {
  ...
  async runReviewCheck(pdfIdentifier, rubricItems) {
  ...
  async getReflections(pdfIdentifier) {
    const params = new URLSearchParams({ pdf_identifier: pdfIdentifier });
    const resp = await fetch(`${this.baseUrl}/reflections/?${params}`);
    if (!resp.ok) await this._throwApiError(resp);
    return resp.json();
  }

  async createReflection(reflectionData) {
    const resp = await fetch(`${this.baseUrl}/reflections/`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async recleanReflection(reflectionId, pdfIdentifier) {
    const params = new URLSearchParams({ pdf_identifier: pdfIdentifier });
    const resp = await fetch(`${this.baseUrl}/reflections/${reflectionId}/reclean?${params}`, { method: 'PUT' });
    if (!resp.ok) await this._throwApiError(resp);
    return resp.json();
  }

  async updateReflection(reflectionId, pdfIdentifier, updates) {
    const params = new URLSearchParams({ pdf_identifier: pdfIdentifier });
    const resp = await fetch(`${this.baseUrl}/reflections/${reflectionId}?${params}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(updates),
    });
  ...
  async deleteReflection(reflectionId, pdfIdentifier) {
    const params = new URLSearchParams({ pdf_identifier: pdfIdentifier });
    const resp = await fetch(`${this.baseUrl}/reflections/${reflectionId}?${params}`, { method: 'DELETE' });
    if (!resp.ok) await this._throwApiError(resp);
  }
  ...
  // Status of the global .bib library configured in Settings; returns
  // { configured, ok, error, entry_count, skipped_count, path }.
  async getBibStatus() {
    const resp = await fetch(`${this.baseUrl}/bib/status`);
    if (!resp.ok) await this._throwApiError(resp);
    return resp.json();
  }

  // Search the .bib library; returns { results: [{ key, authors, title,
  // year, venue, link }], total }.
  async searchBib(q) {
    const params = new URLSearchParams({ q });
    const resp = await fetch(`${this.baseUrl}/bib/search?${params}`);
    if (!resp.ok) await this._throwApiError(resp);
    return resp.json();
  }
