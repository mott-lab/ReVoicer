...
/**
 * Absolute directory of the currently open PDF, decoded from the app URL's
 * `?file=` param (see getPdfBaseName). Returns '' when the path can't be
 * decoded back to a filesystem path.
 */
function getPdfDirectory() {
  ...
  const seg = fileParam.split('#')[0].split('?')[0].split('/').pop() || '';
  const decodedPath = seg ? decodePdfPathSegment(seg) : null;
  if (!decodedPath || !/[\\/]/.test(decodedPath)) return '';
